...
from NeuralNetwork.layer import Layer, InputLayer
...
class NeuralNetwork:
    ...
    def __init__(self, name: str, bias_momentum: float, weight_momentum: float, test_batch: Batch = None,
        ...
        self.graph_name: str = f'{self._name}_graph'
        self.create_log: bool = True
        self.log_name: str = f'{self._name}_log'
    ...
    def add_layer(self, size: int, activation_func: str) -> None:
        ...
        if layer_index == 0:
            self.layers.append(
                InputLayer(index=layer_index, size=size, activation_func_name=activation_func))
        else:
            prev_size = self.layers[layer_index - 1].size
            self.layers.append(
    ...
    def graph(self, save_graph: bool = True, temp: bool = False) -> None:
        """
        ...
        if temp:
            ...
            x = [x for x in range(len(self.__mistakes_log__)+1)]
            y = self.__mistakes_log__.copy()
            y.append(utils.batch_mistake_value(net=self, batch=self.test_batch))
            print(x,y)
        else:
        ...
        if save_graph:
            plt.savefig(f'{self._graph_location}\\{self.graph_name}{"_temp" if temp else ""}')
...
def restore_net(location, name) -> NeuralNetwork:
    ...
    for layer in net:
        ...
        weights = utils.str2tensor(weights.strip('WEIGHTS: '))
        biases = utils.str2tensor(biases.strip('BIASES: '))
